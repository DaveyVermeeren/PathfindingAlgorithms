...
bool DFS::IsValid(std::vector<std::vector<Node>> Visited, int row, int col)
{
...
void DFS::Traverse(std::vector<std::vector<ImageTile>>& Map, int row, int col)
{
    if (Searching)
    {
        if (!First)
